...
class _fast3tree_lib():
    def __init__(self, dim, use_double):
        ...
        mytype = np.dtype([('idx', np.int64), ('pos', float_dtype, self.dim)], align=True)
...
class fast3tree:
    ...
    def _load_data(self, data, raw=False):
        if raw:
            if self._lib is None:
                dt = data.dtype[1]
                self._lib = _fast3tree_lib(dt.shape[1], (dt.type == np.float64))
            if data.dtype != self._lib.input_dtype:
            ...
            if self._lib is None:
                self._lib = _fast3tree_lib(s[1], \
                        self._force_double or (data.dtype.type != np.float32))
    ...
    def rebuild(self, data=None, raw=False):
        ...
        if data is not None:
            self._load_data(data, raw)
        self._lib.run.fast3tree_rebuild(self._tree_ptr, \
                np.int64(self.data.shape[0]), self.data)
    ...
    def query_nearest_distance(self, center):
        ...
        d = self._lib.run.fast3tree_find_next_closest_distance(self._tree_ptr, \
    ...
    def query_radius(self, center, r, periodic=False, output='index'):
        ...
        if periodic:
            __ = self._lib.run.fast3tree_find_sphere_periodic(self._tree_ptr, \
                    self._res_ptr, center_arr, self._lib.float_dtype(r))
        else:
            self._lib.run.fast3tree_find_sphere(self._tree_ptr, self._res_ptr, \
                    center_arr, self._lib.float_dtype(r))
    ...
    def query_box(self, corner1, corner2, inside=True, output='index'):
        ...
        if inside:
            self._lib.run.fast3tree_find_inside_of_box(self._tree_ptr, \
